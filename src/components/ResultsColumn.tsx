...
function ResultCard({
  ...
  const hasMoreContent = result.summary || result.highlights?.length || result.meta;
  const snippetIsTruncated = result.snippet && result.snippet.length > 150;
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Snippet */}
          {result.snippet && (
            <p className={`text-sm text-zinc-400 mt-2 ${!expanded && snippetIsTruncated ? 'line-clamp-2' : ''}`}>
              {result.snippet}
            </p>
          )}
          ...
          {expanded && (
            <div className="mt-3 space-y-3">
              ...
              {/* All highlights (Exa) */}
              {result.highlights && result.highlights.length > 1 && (
                <div className="text-sm">
                  <div className="text-xs text-zinc-500 uppercase mb-1">Highlights</div>
                  <ul className="space-y-1">
                    {result.highlights.map((h, i) => (
                      <li key={i} className="text-zinc-400 text-sm pl-2 border-l border-zinc-700">
                        {h}
                      </li>
                    ))}
                  </ul>
              ...
              {/* Meta section */}
              {result.meta && (
                <div className="text-xs bg-zinc-900/50 rounded p-2 space-y-1">
                  <div className="text-zinc-500 uppercase mb-1">Meta</div>
                  {result.meta.id && (
                    <div className="text-zinc-500">ID: <span className="text-zinc-400 font-mono">{result.meta.id}</span></div>
                  )}
                  {result.meta.score !== undefined && (
                  ...
                  {result.meta.hostname && (
                    <div className="text-zinc-500">Hostname: <span className="text-zinc-400">{result.meta.hostname}</span></div>
                  )}
                  {result.meta.isLocal && (
...
export function ResultsColumn({ provider, results, otherResults, metrics, otherMetrics, isLoading, error }: Props) {
  ...
  // Determine if this provider is faster
